...
class FrontendScanRequest(_BaseModel):
...
def frontend_start_scan(request: Request, body: FrontendScanRequest) -> dict:
    """Start a scan — same execution path as ``POST /ui/scan/run`` (parallel per entity).

    Each entity uses ``resolve_scan_start`` + ``build_scan_windows`` and runs
    ``run_scan_worker`` on the shared ``entity_executor`` with the global rate
    limiter / HTTP client. Each successful window already runs narrative
    generation inside ``run_entity_incremental`` (``SQLRunNarrative``).
    """
    from bigdata_briefs.api.routes.universes import _UNIVERSES
    ...
    if body.universe:
        entity_ids = list(_UNIVERSES.get(body.universe) or [])
    elif body.entity_id:
        entity_ids = [body.entity_id]
    else:
        return {"error": "provide entity_id or universe"}

    if not entity_ids:
        return {"error": f"universe '{body.universe}' not found or empty"}

    try:
        requested_start = datetime.strptime(body.start_date.strip(), "%Y-%m-%d").replace(
            hour=0, minute=0, second=0, tzinfo=timezone.utc
        )
    except ValueError:
    ...
    if body.end_date and body.end_date.strip():
        try:
            end = datetime.strptime(body.end_date.strip(), "%Y-%m-%d").replace(
                hour=23, minute=59, second=59, tzinfo=timezone.utc
            )
        except ValueError:
    ...
    engine = get_engine()
    executor = request.app.state.entity_executor
    rate_limiter = request.app.state.bigdata_rate_limiter
    connection_sem = request.app.state.bigdata_connection_sem
    http_client = request.app.state.bigdata_http_client

    scanned_entity_ids: list[str] = []
    total_windows = 0
    ...
    if not scanned_entity_ids:
        return {
            "error": "No windows to process — entities are already up to date for this range.",
        }

    end_str = body.end_date.strip() if body.end_date and body.end_date.strip() else end.strftime("%Y-%m-%d")

    return {
        "status": "started",
        "entity_ids": scanned_entity_ids,
...
def get_scan_status(entity_ids: str, start_date: str, end_date: str) -> dict:
    """Poll scan progress: returns per-entity per-day status."""
    from bigdata_briefs.api.routes.scan import resolve_scan_start
    ...
    with Session(engine) as session:
        ...
        for entity_id in ids:
            ...
            for date_str in all_dates:
                td_obj = _date.fromisoformat(date_str)
                # Match entity_runner / narrative aggregation: UTC calendar day bounds.
                day_start = datetime(
                    td_obj.year, td_obj.month, td_obj.day, 0, 0, 0, tzinfo=timezone.utc
                )
                day_end = datetime(
                    td_obj.year,
                    td_obj.month,
                ...
                if run is None:
                    # Resume skips only when there is truly no run for this day yet.
                    ...
                    day_results.append({
                        "date": date_str, "status": "succeeded",
                        "saved": active,
                    ...
                    day_results.append({"date": date_str, "status": "failed",
    ...
    completed = sum(
        1 for r in results for d in r["days"]
        if d["status"] in ("succeeded", "failed", "skipped")
    )
...
def get_update_preview(
    ...
    """Return per-entity resume points for the Update view preview.

    For each entity the response contains:
    - last_run_at  : ISO-8601 of the most-recent process_completed_at_utc
    - resume_from  : ISO-8601 of the effective scan-start (output of resolve_scan_start)
    - est_windows  : calendar-day windows that would be submitted right now
    - has_history  : False when no prior run exists (entity is skipped by Update)
    """
    from bigdata_briefs.api.routes.scan import (
        build_scan_windows,
        resolve_scan_start,
    ...
    if scope == "entity":
        if not entity_id:
            return {"error": "entity_id required for scope=entity"}
        eids = [entity_id]
    elif scope == "universe":
        if not universe:
            return {"error": "universe required for scope=universe"}
    ...
    with Session(engine) as session:
        for eid in eids:
            ...
            # Use the start of the last-run day as requested_start so resolve_scan_start
            # can correctly pick up mid-day resume points.
...
class UpdateRunRequest(_BaseModel):
    scope: str  # entity | universe | all
    entity_id: str | None = None
    universe: str | None = None
...
def frontend_start_update(request: Request, body: UpdateRunRequest) -> dict:
    """Catch-up scan: resumes every entity in scope from its last run to now."""
    from bigdata_briefs.api.routes.scan import (
        build_scan_windows,
        db_create_scan,
        resolve_scan_start,
    ...
    if body.scope == "entity":
        ...
        eids = [body.entity_id]
    elif body.scope == "universe":
        if not body.universe:
            return {"error": "universe required"}
        eids = list(_UNIVERSES.get(body.universe) or [])
        if not eids:
            return {"error": f"universe '{body.universe}' not found or empty"}
    elif body.scope == "all":
        with Session(engine) as session:
        ...
        return {"error": f"unknown scope '{body.scope}'"}
    ...
    scanned_entity_ids: list[str] = []
    total_windows = 0
    earliest_resume: date_cls | None = None
    ...
    if not scanned_entity_ids:
        return {"error": "No windows to process — all entities are already up to date."}

    end_str = now.strftime("%Y-%m-%d")
    start_str = earliest_resume.isoformat() if earliest_resume else now.strftime("%Y-%m-%d")
