...
class RequestsPerMinuteController:
    ...
    def acquire(self) -> None:
        """Synchronous token acquire: block (with retry/sleep) until the
        rate-limit window has room, then record the request.

        Shares the same deque + Lock as :meth:`aacquire`, so any mix of
        sync and async callers competes for a single QPM budget.

        Raises
        ------
        TooManyAPIRetriesError
            if the limiter is saturated for longer than
            ``MAX_RETRIES_RATE_LIMITER * time_before_retry`` seconds.
        """
    ...
    async def aacquire(self) -> None:
        """Async token acquire for ``httpx.AsyncClient`` / LangGraph nodes.

        Identical semantics to :meth:`acquire` but yields to the event loop
        between retries via ``asyncio.sleep`` instead of blocking the thread
        with ``time.sleep``. Both variants update the same deque + Lock, so
        sync and async traffic share one 450 QPM budget.
        """
        for idx in range(MAX_RETRIES_RATE_LIMITER):
            if self._allowed_by_rate_limit():
                return
            ...
            await asyncio.sleep(self.time_before_retry)
