...
curl -s -X POST http://localhost:8000/api/v1/batch/run-parallel \
    ...
    -d "{\"universe\":\"my_portfolio\",\"force_window_start\":\"${START}\",\"force_window_end\":\"${END}\",\"categories\":[\"news\"],\"generate_narrative\":true,\"compute_signals\":true,\"ranking_metric\":\"media_attention_momentum\"}"
